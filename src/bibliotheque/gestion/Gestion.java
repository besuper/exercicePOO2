...
public class Gestion {
    ...
    private void gestLocations() {
        for(int i =  0; i < lex.size(); i++) {
            Exemplaire exp = lex.get(i);

            if (!exp.enLocation()) {
                System.out.println((i+1) + ". " + exp.getOuvrage().getTitre());
            }
        }

        System.out.println("Choix de l'examplaire ");
        int choixExp = sc.nextInt();

        for(int i =  0; i < llect.size(); i++) {
            System.out.println((i+1) + ". " + llect.get(i).getNom());
        }

        System.out.println("Choix du lecteur ");
        int choixLect = sc.nextInt();

        lloc.add(new Location(llect.get(choixLect - 1), lex.get(choixExp - 1)));
    }
    ...
    private void gestOuvrages() {
        ...
        switch (choix) {
            case 1:
                ...
                TypeLivre tl = TypeLivre.valueOf(type.toUpperCase());
            ...
            case 2:
                ...
                o = new DVD(titre, ageMin, dp, ploc, langue, genre, code, duree, nbreBonus);
                break;
            case 3:
                System.out.println("code ");
                ...
                System.out.println("duree totale ");
                String dureeTotale = sc.next();

                System.out.println("plages ");
                byte nbrPlages = sc.nextByte();

                sc.skip("\n");

                o = new CD(titre, ageMin, dp, ploc, langue, genre, codeCD, nbrPlages, dureeTotale);
                break;
        }
        louv.add(o);
