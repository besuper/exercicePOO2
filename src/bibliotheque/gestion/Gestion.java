...
public class Gestion {
    ...
    private void gestRestitution() {
        List<Exemplaire> lex_loue = new ArrayList<>();
        ...
        System.out.println(lex_loue);

        int choix = Utilitaire.choixListe(lex_loue);

        System.out.println("Date restitution ");
        LocalDate restitution = Utilitaire.lecDate();

        Exemplaire exp = lex_loue.get(choix - 1);

        exp.getLloc().get(exp.getLloc().size() - 1).setDateRestitution(restitution);
    }
    ...
    private void gestLocations() {
        ...
        for (Exemplaire e : lex) {
            if (e.enLocation()) continue;

            disponibles.add(e);
        }

        disponibles.sort(new Exemplaire.ComparatorMatricule());

        choix = Utilitaire.choixListe(disponibles);

        Exemplaire ex = disponibles.get(choix - 1);
    ...
    private void gestRayons() {
        ...
        List<Exemplaire> exp = new ArrayList<>();

        for(Exemplaire e : lex) {
            if(e.getRayon().equals(r)) {
                if(exp.contains(e)) continue;
                r.addExemplaire(e);
        ...
        r.listerExemplaires().sort(new Exemplaire.ComparatorTitreOuvrage());
    ...
    private void gestExemplaires() {
        ...
        for(Rayon r : lrayon) {
            if (r.listerExemplaires().contains(ex)) {
                ex.setRayon(r);
            }
        }
    }
    ...
    private void gestOuvrages() {
     ...
     /*switch(choix) {
        ...
        List<Auteur> auts = new ArrayList<>();

        for(Auteur a : laut) {
            if(a.getLouvrage().contains(o)) {
                if(auts.contains(a)) continue;
    ...
    private void gestAuteurs() {
        ...
        Collections.sort(a.getLouvrage());
    }
